...
//            }
        ...
        private fun initTts() {
            tts = TextToSpeech(context) { status ->
                Log.e("TTS", "onInit status = $status")
                if (status == TextToSpeech.SUCCESS) {
                    ...
                    if (result == TextToSpeech.LANG_MISSING_DATA ||
                        ...
                        Log.e("TTS", "当前设备不支持中文 TTS")
                    } else {
                        Log.d("TTS", "TTS 初始化成功（中文）")
                    }
                } else {
                    Log.e("TTS", "TTS 初始化失败")
                }
...
//            replayAudioButton.tag = data
        ...
        override fun onLongClick(v: View): Boolean {
            ...
            PopupWindowHelper().showPopupWindow(
                ...
                if (v.id == R.id.assistant_text_read) {
                    speak(textView.text.toString())
                } else if (v.id == R.id.assistant_text_read_stop) {
                    tts.stop()
                }
//                } else if (v.id == R.id.assistant_text_copy) {
